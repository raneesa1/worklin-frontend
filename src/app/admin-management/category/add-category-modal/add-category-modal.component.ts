...
export class AddCategoryModalComponent implements OnInit {
  ...
  private fetchSkills() {
    this.adminManagementService.getSkillsForCategory().subscribe({
      next: (response: any) => {
        if (response && Array.isArray(response.skills)) {
          this.skills = response.skills;
          console.log(this.skills, 'consoling the skills from fetchskills');
        } else {
          console.error('Skills data is not in the expected format:', response);
        }
      },
      error: (err: any) => {
  ...
  saveCategory(event: Event) {
    ...
    if (this.categoryForm.valid) {
      ...
      this.adminManagementService.addCategory(category).subscribe({
        next: (response) => {
