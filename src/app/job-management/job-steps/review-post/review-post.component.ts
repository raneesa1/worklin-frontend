...
import { ConfirmJobPostModalComponent } from '../../confirm-job-post-modal/confirm-job-post-modal.component';
...
export class ReviewPostComponent {
  ...
  sendDataToBackend() {
    ...
    if (jobPost) {
      ...
      this.jobService.sendJobPostData(jobPostData).subscribe(
        (response) => {
          this.router.navigate(['job/job-posted']);
